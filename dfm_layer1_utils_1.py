...
def _extend_monthly_comparison_to_impact_date(df: pd.DataFrame, impact_date) -> pd.DataFrame:
    """
    Extend the updated monthly comparison panel so the requested news impact
    month is explicitly present on the index.

    This is necessary for mixed-frequency DynamicFactorMQ models because the
    internal state-space representation can fall back to a generated integer
    index when the combined monthly / quarterly labels are non-monotonic. In
    that case, statsmodels does not reliably interpret an out-of-sample Period
    label as extendable during `results.news(...)`; the impact month must
    already be present on the updated comparison dataset that is passed into the
    method.

    The extension is calendar-consistent:
    - monthly PeriodIndex inputs are extended with monthly Period labels;
    - monthly DatetimeIndex inputs are extended at month-start for "MS"-style
      indexes and month-end for plain "M" indexes.

    The function never rewrites first-day timestamps to month-end unless the
    existing DatetimeIndex is already month-end based.
    """
    out = df.copy()
    idx = out.index
    ...
    if isinstance(idx, pd.PeriodIndex):
        freqstr = str(getattr(idx, "freqstr", "") or "")
        if not freqstr.startswith("M"):
            raise ValueError(
                "Updated comparison data must use a monthly PeriodIndex for the "
                f"news decomposition. Got freq={freqstr!r}."
            )
        if isinstance(impact_date, pd.Period):
            impact_period = impact_date.asfreq(idx.freq)
        else:
            impact_period = parse_periodish(impact_date, freq_hint="M")
            if impact_period is None:
                raise ValueError(
                    "Could not parse the requested impact_date to a monthly Period "
                    f"for comparison-index extension: {impact_date!r}."
                )
            impact_period = impact_period.asfreq(idx.freq)

        if impact_period not in idx:
            full_index = pd.period_range(start=idx.min(), end=max(idx.max(), impact_period), freq=idx.freq)
    ...
    if isinstance(idx, pd.DatetimeIndex):
        ...
        if freqstr is None:
            raise ValueError(
                "Updated comparison data must have a fixed or inferable monthly "
                "DatetimeIndex to extend the news impact month."
            )
        freq_upper = str(freqstr).upper()
        if not freq_upper.startswith("M"):
            raise ValueError(
                "Updated comparison DatetimeIndex must be monthly for the news "
                f"decomposition. Got freq={freqstr!r}."
            )

        if isinstance(impact_date, pd.Period):
            impact_period = impact_date.asfreq("M")
        else:
            impact_period = parse_periodish(impact_date, freq_hint="M")
            if impact_period is None:
                raise ValueError(
        ...
        how = "end" if freq_upper.startswith("M") and not freq_upper.startswith("MS") else "start"
        impact_timestamp = impact_period.to_timestamp(how=how)
        if impact_timestamp not in idx:
            full_index = pd.date_range(start=idx.min(), end=max(idx.max(), impact_timestamp), freq=freqstr)
            out = out.reindex(full_index)
        return out

    raise TypeError(
        "Updated comparison data must have a PeriodIndex or DatetimeIndex. "
...
def _assert_news_impact_date_supported(index: pd.Index, impact_date, *, vintage: pd.Period, target_quarter: pd.Period) -> None:
    """
    Fail fast with a clear message if the requested news impact date is not
    explicitly present on the updated comparison index passed to statsmodels.

    A looser `get_index_loc(...)` check is insufficient here because a plain
    pandas index may appear extendable even when the mixed-frequency model that
    statsmodels builds internally cannot resolve the requested label.
    """
    try:
        supported = bool(impact_date in index)
    except Exception:
        supported = False
    ...
    raise ValueError(
        "News impact date is not present on the updated comparison index that "
        "is passed into statsmodels. "
...
def run_layer1_dfm(config: ProtocolConfig) -> Dict[str, pd.DataFrame]:
    ...
    for vintage in vintage_schedule:
        ...
        if prev_results is not None and prev_vintage is not None:
            ...
            comparison_monthly_model = _extend_monthly_comparison_to_impact_date(
