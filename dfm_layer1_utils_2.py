...
def _coerce_impact_date_for_model_index(index_or_model, target_quarter: pd.Period):
    """
    Map the quarter-end impact month to the actual label type used by the
    statsmodels prediction index.

    Accepts either a raw Pandas index, a statsmodels model instance, or a
    statsmodels results object. This avoids validating against an external
    DataFrame index when news() will actually resolve dates against the
    internal model index / row labels.
    """
    impact_month = get_quarter_end_month(target_quarter)

    model = _extract_prediction_model(index_or_model)
    if model is not None:
        internal_index = getattr(model, "_index", None)
        row_labels = getattr(getattr(model, "data", None), "row_labels", None)
        for index in (internal_index, row_labels):
        ...
        raise TypeError(
            "Could not infer a date-like prediction index from the statsmodels model. "
            f"internal_index_type={type(internal_index).__name__}, row_labels_type={type(row_labels).__name__}."
        )
    ...
    if isinstance(index, pd.DatetimeIndex):
        ...
        return impact_month.to_timestamp(how="start")
    raise TypeError(
        "News comparison index must be a PeriodIndex, DatetimeIndex, statsmodels model, or statsmodels results object; "
        f"got {type(index_or_model).__name__}."
    )
...
def _assert_news_impact_date_supported(index_or_model, impact_date, *, vintage: pd.Period, target_quarter: pd.Period) -> None:
    ...
    extend the requested impact date on the *actual* model prediction index.

    For raw Pandas indexes, fall back to get_index_loc. For statsmodels models
    / results objects, call the same _get_prediction_index machinery that
    news() ultimately relies on.
    """
    model = _extract_prediction_model(index_or_model)
    if model is not None:
        try:
            ...
            row_labels = getattr(getattr(model, "data", None), "row_labels", None)
            debug = {}
            debug.update(_index_debug_dict(internal_index, "internal_index"))
            debug.update(_index_debug_dict(row_labels, "row_labels"))
            raise ValueError(
                "News impact date is incompatible with the actual statsmodels prediction index. "
...
def select_factor_order(
    ...
    quarterly_target_model = align_quarterly_target_to_monthly_panel(
        monthly_panel_model,
        quarterly_target,
...
def fit_dfm_single_vintage(
    ...
    quarterly_target_model = align_quarterly_target_to_monthly_panel(
        monthly_panel_model,
        quarterly_target[[quarterly_target_name]] if quarterly_target_name in quarterly_target.columns else quarterly_target,
...
def make_diagnostics_row(
    ...
    diag.update(_index_debug_dict(internal_index, "model_index"))
    diag.update(_index_debug_dict(row_labels, "row_labels"))
    return diag
...
def align_quarterly_target_to_monthly_panel(
    monthly_panel: pd.DataFrame,
    quarterly_target: pd.DataFrame,
    quarterly_target_name: str = "gdp_growth",
) -> pd.DataFrame:
    """
    Align quarterly target history to the first quarter supported by the
    monthly panel before passing it into DynamicFactorMQ.

    This preserves the repository's first-day period semantics while avoiding a
    statsmodels mixed-frequency edge case: if quarterly history begins before
    the monthly panel, the internally constructed monthly prediction index can
    become non-monotonic and later forecasting / news() calls will fall back to
    a generated integer index.
    """
    monthly_panel_model = as_model_index(monthly_panel)
    quarterly_target_model = as_model_index(quarterly_target)

    if not isinstance(monthly_panel_model.index, pd.PeriodIndex) or not str(monthly_panel_model.index.freqstr).startswith("M"):
        raise ValueError(
            "Monthly panel must have a monthly PeriodIndex before mixed-frequency alignment. "
            f"Got {type(monthly_panel_model.index).__name__} / {getattr(monthly_panel_model.index, 'freqstr', None)!r}."
        )
    if not isinstance(quarterly_target_model.index, pd.PeriodIndex) or not str(quarterly_target_model.index.freqstr).startswith("Q"):
        raise ValueError(
            "Quarterly target must have a quarterly PeriodIndex before mixed-frequency alignment. "
            f"Got {type(quarterly_target_model.index).__name__} / {getattr(quarterly_target_model.index, 'freqstr', None)!r}."
        )

    first_supported_quarter = monthly_panel_model.index.min().asfreq("Q")
    aligned = quarterly_target_model.loc[quarterly_target_model.index >= first_supported_quarter].copy()
    aligned = aligned.sort_index()
    aligned = aligned[~aligned.index.duplicated(keep="last")]

    if aligned.empty:
        raise ValueError(
            "Quarterly target became empty after alignment to the monthly panel coverage. "
            f"monthly_start={monthly_panel_model.index.min()}, first_supported_quarter={first_supported_quarter}, "
            f"quarterly_min={quarterly_target_model.index.min()}, quarterly_max={quarterly_target_model.index.max()}."
        )

    if quarterly_target_name in quarterly_target_model.columns and quarterly_target_name not in aligned.columns:
        aligned[quarterly_target_name] = quarterly_target_model[quarterly_target_name]

    return aligned


def _extract_prediction_model(index_or_model):
    """
    Return the concrete statsmodels model object used for prediction-index
    resolution, accepting either a model, a results object, or a plain index.
    """
    if hasattr(index_or_model, "_get_prediction_index"):
        return index_or_model
    model = getattr(index_or_model, "model", None)
    if model is not None and hasattr(model, "_get_prediction_index"):
        return model
    return None


def _index_debug_dict(index: Optional[pd.Index], prefix: str) -> Dict[str, Any]:
    if index is None:
        return {
            f"{prefix}_type": None,
            f"{prefix}_freq": None,
            ...
            f"{prefix}_end": None,
            f"{prefix}_len": 0,
            f"{prefix}_monotonic": None,
        }

    info = {
        f"{prefix}_type": type(index).__name__,
        f"{prefix}_freq": getattr(index, "freqstr", None),
        f"{prefix}_len": int(len(index)),
        f"{prefix}_monotonic": bool(getattr(index, "is_monotonic_increasing", True)),
    }
    try:
        info[f"{prefix}_start"] = index.min() if len(index) else None
    except Exception:
        info[f"{prefix}_start"] = None
    try:
        info[f"{prefix}_end"] = index.max() if len(index) else None
    except Exception:
        info[f"{prefix}_end"] = None
    return info


def model_index_debug_frame(index_or_model, label: str = "model") -> pd.DataFrame:
    """
    Summarize the actual statsmodels prediction index and row labels used by
    forecasting / news() resolution.
    """
    model = _extract_prediction_model(index_or_model)
    if model is not None:
        internal_index = getattr(model, "_index", None)
        row_labels = getattr(getattr(model, "data", None), "row_labels", None)
        data = {"label": label}
        data.update(_index_debug_dict(internal_index, "internal_index"))
        data.update(_index_debug_dict(row_labels, "row_labels"))
        return pd.DataFrame([data])

    raw_index = index_or_model if isinstance(index_or_model, pd.Index) else None
    data = {"label": label}
    data.update(_index_debug_dict(raw_index, "raw_index"))
    return pd.DataFrame([data])
...
def run_layer1_dfm(config: ProtocolConfig) -> Dict[str, pd.DataFrame]:
    ...
    for vintage in vintage_schedule:
        ...
        quarterly_target_for_model = align_quarterly_target_to_monthly_panel(
            monthly_panel,
            quarterly_target[["gdp_growth"]] if "gdp_growth" in quarterly_target.columns else quarterly_target,
            quarterly_target_name="gdp_growth",
        )
        ...
        if config.select_factor_order_per_vintage:
            ...
            factor_order = select_factor_order(
                ...
                quarterly_target=quarterly_target_for_model[["gdp_growth"]],
                factors=factors,
        ...
        model, results = fit_dfm_single_vintage(
            monthly_panel=monthly_panel,
            quarterly_target=quarterly_target_for_model,
        ...
        if prev_results is not None and prev_vintage is not None:
            ...
            comparison_quarterly_target = align_quarterly_target_to_monthly_panel(
                comparison_monthly_model,
                quarterly_target_for_model[["gdp_growth"]] if "gdp_growth" in quarterly_target_for_model.columns else quarterly_target_for_model,
                quarterly_target_name="gdp_growth",
            )

            comparison_results = prev_results.apply(
                comparison_monthly_model,
                endog_quarterly=comparison_quarterly_target,
                refit=False,
    ...
    for bench_name in ["spf_mean", "spf_median"]:
        if bench_name in target_objects and "target_quarter" in nowcasts_df.columns:
            nowcasts_df = nowcasts_df.merge(
                target_objects[bench_name],
                left_on="target_quarter",
                right_on="quarter",
                how="left",
            ).drop(columns=["quarter"])
            nowcasts_df[f"dfm_minus_{bench_name}"] = nowcasts_df["dfm_nowcast"] - nowcasts_df[bench_name]

    nowcasts_df["dfm_residual_lag1_third_release"] = nowcasts_df.get("dfm_residual_third_release", pd.Series(index=nowcasts_df.index, dtype=float)).shift(1)
    nowcasts_df["dfm_residual_lag1_latest_rtdsm"] = nowcasts_df.get("dfm_residual_latest_rtdsm", pd.Series(index=nowcasts_df.index, dtype=float)).shift(1)

    # Export artifacts
    ensure_directory(output_dir)
    catalog.to_csv(output_dir / "repository_catalog.csv", index=False)
    md_manifest.to_csv(output_dir / "vintage_manifest_monthly.csv", index=False)
    qd_manifest.to_csv(output_dir / "vintage_manifest_quarterly.csv", index=False)
    nowcasts_df.to_csv(output_dir / "dfm_nowcasts.csv", index=False)
    news_series_df.to_csv(output_dir / "dfm_news_series.csv", index=False)
    news_blocks_df.to_csv(output_dir / "dfm_news_blocks.csv", index=False)
    coverage_df.to_csv(output_dir / "dfm_coverage.csv", index=False)
    diagnostics_df.to_csv(output_dir / "dfm_diagnostics.csv", index=False)

    try:
        states_df.to_parquet(output_dir / "dfm_states.parquet", index=False)
    except Exception:
        states_df.to_csv(output_dir / "dfm_states.csv", index=False)
    ...
    return {
        ...
        "vintage_manifest_monthly": md_manifest,
        "vintage_manifest_quarterly": qd_manifest,
    }
