...
"""
Final merged helper utilities for Layer 1 of the hybrid DFM + ML nowcasting project.

This module is repository-aware, vintage-aware, and careful about the
repository\'s first-day-of-month / first-day-of-quarter timestamp convention.
It avoids artificial month-end conversion, canonicalizes time semantics with
pandas PeriodIndex objects, preserves full quarterly history in the
mixed-frequency endog construction, and exposes the notebook-facing diagnostics
needed for the definitive Layer-1 pipeline.
"""
...
def model_index_debug_frame(results_or_model, label: Optional[str] = None) -> pd.DataFrame:
    """
    Backward-compatible compact audit view used by the candidate notebooks.
    Accepts either a results object or a model object.
    """
    model = getattr(results_or_model, "model", results_or_model)
    out = model_index_audit_frame(model).copy()
...
def _prediction_index_from_model(model_or_index) -> pd.Index:
    """
    Backward-compatible wrapper retained for notebook diagnostics and any older
    call sites. It resolves to the actual statsmodels-supported prediction index
    whenever possible.
    """
    return _prediction_index_object(model_or_index)


def model_prediction_index_audit(model, impact_date=None) -> pd.DataFrame:
    """
    Human-readable audit of the supported statsmodels prediction index and the
    original row labels, with optional impact-date resolution diagnostics.
    """
    supported_index = getattr(model, "_index", None)
    row_labels = getattr(getattr(model, "data", None), "row_labels", None)

    out: Dict[str, Any] = {
        "supported_index_type": type(supported_index).__name__ if supported_index is not None else None,
        "supported_index_freq": getattr(supported_index, "freqstr", None) if supported_index is not None else None,
        "supported_index_generated": bool(getattr(model, "_index_generated", False)),
        "supported_index_monotonic": (
            bool(supported_index.is_monotonic_increasing)
            if isinstance(supported_index, (pd.PeriodIndex, pd.DatetimeIndex))
            else None
        ),
        "supported_index_min": supported_index.min() if isinstance(supported_index, pd.Index) and len(supported_index) else None,
        "supported_index_max": supported_index.max() if isinstance(supported_index, pd.Index) and len(supported_index) else None,
        ...
        "row_labels_monotonic": (
            bool(row_labels.is_monotonic_increasing)
        ...
        "prediction_loc_start": None,
        "prediction_loc_end": None,
        "prediction_oos": None,
...
def make_diagnostics_row(
    ...
    """
    One-row diagnostic summary for a fitted vintage. This merges the richer
    index diagnostics from helper v3 with the preserved-history model-index
    logic from helper v4.
    """
    llf_path = np.asarray(results.mle_retvals.get("llf", []), dtype=float)
    supported_index = getattr(results.model, "_index", None)
    ...
    return {
        ...
        "n_monthly_obs": int(monthly_panel.notna().sum().sum()),
        "factor_order": factor_order,
        "llf_final": float(results.llf),
        "em_iterations": int(results.mle_retvals.get("iter", len(llf_path))),
